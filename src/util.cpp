...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "vida" is a composite category enabling all VIDA-related debug output
            if (ptrCategory->count(std::string("vida"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\VIDA
// Windows >= Vista: C:\Users\Username\AppData\Roaming\VIDA
// Mac: ~/Library/Application Support/VIDA
// Unix: ~/.vida
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VIDA";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "vida.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty vida.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override vida.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "vidad.pid"));
