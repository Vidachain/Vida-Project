                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zVDA ought to be enough for anybody." - Bill Gates, 2017
    ui->zVDApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzVDASyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zVDApayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzVDA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVDA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zVDA, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVDA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVDA in ") +
...
void PrivacyDialog::on_pushButtonSpendzVDA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVDA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zVDA, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zVDA
        sendzVDA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zVDA
    sendzVDA();
}

void PrivacyDialog::on_pushButtonZVdaControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZVdaControlDialog* zVdaControl = new ZVdaControlDialog(this);
    zVdaControl->setModel(walletModel);
    zVdaControl->exec();
}

void PrivacyDialog::setZVdaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVdaSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVDA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Vida Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zVDApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zVDA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VDA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVDA</b>";
    ...
    // use mints from zVDA selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZVdaControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZVdaControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zVDA transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zVDA was spent successfully update the addressbook with the label
    ...
    // Clear zvda selector in case it was used
    ZVdaControlDialog::setSelectedMints.clear();
    ui->labelzVdaSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVDA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Vda, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zVDA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zvdaTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVDA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zVDA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zVDA "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zVDA "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zVDA </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zVDA </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zVDA </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzVDASyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzVDA->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zVDA
        //ui->pushButtonMintzVDA->setEnabled(false);
        //ui->pushButtonMintzVDA->setToolTip(tr("zVDA is currently disabled due to maintenance."));

        // Spend zVDA
        ui->pushButtonSpendzVDA->setEnabled(false);
        ui->pushButtonSpendzVDA->setToolTip(tr("zVDA is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zVDA
        //ui->pushButtonMintzVDA->setEnabled(true);
        //ui->pushButtonMintzVDA->setToolTip(tr("PrivacyDialog", "Enter an amount of VDA to convert to zVDA", 0));

        // Spend zVDA
        ui->pushButtonSpendzVDA->setEnabled(true);
        ui->pushButtonSpendzVDA->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
