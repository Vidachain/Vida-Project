...
#include "ui_zvdacontroldialog.h"
...
std::set<std::string> ZVdaControlDialog::setSelectedMints;
std::set<CMintMeta> ZVdaControlDialog::setMints;

bool CZVdaControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZVdaControlDialog::COLUMN_DENOMINATION || column == ZVdaControlDialog::COLUMN_VERSION || column == ZVdaControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZVdaControlDialog::ZVdaControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZVdaControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZVdaControlDialog::~ZVdaControlDialog()
{
    delete ui;
}

void ZVdaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVdaControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZVdaControlWidgetItem* itemDenom(new CZVdaControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZVdaControlWidgetItem *itemMint = new CZVdaControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zvdaTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zvdaTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zVDA.";
            else if (!mint.isSeedCorrect)
                strReason = "The zVDA seed used to mint this zVDA is not the same as currently hold in the wallet";
            else
...
void ZVdaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVdaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZVda_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVdaControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZVdaControlDialog::GetSelectedMints()
{
...
void ZVdaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
