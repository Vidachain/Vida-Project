...
void CTxMemPool::removeCoinbaseSpends(const CCoinsViewCache* pcoins, unsigned int nMemPoolHeight)
{
    ...
    for (std::map<uint256, CTxMemPoolEntry>::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        for (const CTxIn& txin : tx.vin) {
			...
			unsigned int nTier = GetMasternodeTierFromOutput(coins->vout[txin.prevout.n].nValue, chainActive.Height());
            if (IsMasternodeOutput(coins->vout[txin.prevout.n].nValue, chainActive.Height())
                && nMemPoolHeight - coins->nHeight < GetMastenodeTierMaturity(nTier)
                && nMemPoolHeight > TIER_BLOCK_HEIGHT)
            {
