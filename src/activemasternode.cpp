...
//
// Bootup the Masternode, look for a 10000 VIDA input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if (GetMasterNodeVin(vin, tier, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            if (!CreateBroadcast(vin, tier, service, keyCollateralAddress, pubKeyCollateralAddress, keyMasternode, pubKeyMasternode, errorMessage, mnb)) {
...
bool CActiveMasternode::CreateBroadcast(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage, CMasternodeBroadcast &mnb, bool fOffline)
{
    ...
    return CreateBroadcast(vin, nTier, CService(strService), keyCollateralAddress, pubKeyCollateralAddress, keyMasternode, pubKeyMasternode, errorMessage, mnb);
}

bool CActiveMasternode::CreateBroadcast(CTxIn vin, unsigned int nTier, CService service, CKey keyCollateralAddress, CPubKey pubKeyCollateralAddress, CKey keyMasternode, CPubKey pubKeyMasternode, std::string& errorMessage, CMasternodeBroadcast &mnb)
{
    ...
    mnb = CMasternodeBroadcast(service, vin, nTier, pubKeyCollateralAddress, pubKeyMasternode, PROTOCOL_VERSION);
    mnb.lastPing = mnp;
...
std::vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    for (const COutput& out : vCoins) {
        if (IsMasternodeOutput(out.tx->vout[out.i].nValue, chainActive.Height())) {
