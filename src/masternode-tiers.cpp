...
unsigned int GetMasternodeTierFromOutput(CAmount nValue, int blockHeight)
{
    ...
    if (blockHeight < TIER_BLOCK_HEIGHT) {
        if (nValue == MASTERNODE_TIER_COINS[MasternodeTiers::TIER_LONG] * COIN) {
            tierRet = MasternodeTiers::TIER_LONG;
        }
    }
    else {
        for (unsigned int tier = MasternodeTiers::TIER_LONG; tier != MasternodeTiers::TIER_NONE; tier++) {
...
{
    if (nTier >= MasternodeTiers::TIER_NONE || nTier < MasternodeTiers::TIER_LONG) {
        return 0;
    }
    else {
...
unsigned int GetMastenodeTierMaturity(unsigned int nTier)
{
    if (nTier >= MasternodeTiers::TIER_NONE || nTier < MasternodeTiers::TIER_LONG) {
        ...
        return MASTERNODE_TIER_COLLATERAL_MATURITY[nTier];
    }
}
...
{
    const unsigned int distribution[MasternodeTiers::TIER_NONE] = {20, 15};
