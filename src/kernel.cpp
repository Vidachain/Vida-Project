...
bool GetHashProofOfStake(const CBlockIndex* pindexPrev, CStakeInput* stake, const unsigned int nTimeTx, const bool fVerify, uint256& hashProofOfStakeRet) {
    ...
    if (fVerify) {
        LogPrint("staking", "%s :{ nStakeModifier=%s\n"
                            ...
                            "}\n",
            __func__, HexStr(modifier_ss), ((stake->IsZVDA()) ? "Not available" : std::to_string(stake->getStakeModifierHeight())));
    }
    return true;
}
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : zVDA stake block is less than allowed start height", __func__);

    if (CZVdaStake* zVDA = dynamic_cast<CZVdaStake*>(stake)) {
        CBlockIndex* pindexFrom = zVDA->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with zVDA stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zVDA stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zVDA->GetValue()));
        if (nChecksum200 != zVDA->GetChecksum())
            return error("%s : accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zVDA->GetChecksum(), nChecksum200);
    } else {
...
bool initStakeInput(const CBlock block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZVdaStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zVDA fails context checks", __func__);
    } else {
        ...
        CVdaStake* vdaInput = new CVdaStake();
        vdaInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(vdaInput);
    }
    return true;
}
