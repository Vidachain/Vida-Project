...
CzVDATracker::CzVDATracker(std::string strWalletFile)
{
...
CzVDATracker::~CzVDATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVDATracker::Init()
{
...
bool CzVDATracker::Archive(CMintMeta& meta)
{
...
bool CzVDATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVDATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVDATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVDATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzVDATracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzVDATracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzVDATracker::GetSerialHashes()
{
...
CAmount CzVDATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVDATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVDATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVDATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVDATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVDATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVDATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVDATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVDATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVDATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzVDATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzVDAWallet* zVDAWallet)
{
    bool iszVDAWalletInitialized = (NULL != zVDAWallet);
    CMintMeta meta;
    ...
    if (! iszVDAWalletInitialized)
        zVDAWallet = new CzVDAWallet(strWalletFile);
    meta.isSeedCorrect = zVDAWallet->CheckSeed(dMint);
    if (! iszVDAWalletInitialized)
        delete zVDAWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzVDATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVDATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVDATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVDATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVDATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVDATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzVDAWallet* zVDAWallet = new CzVDAWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zVDAWallet);
        }
        delete zVDAWallet;
        LogPrint("zero", "%s: added %d dzvda from DB\n", __func__, listDeterministicDB.size());
    }
