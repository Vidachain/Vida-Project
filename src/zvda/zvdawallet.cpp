...
CzVDAWallet::CzVDAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zvda seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVDASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVDASeed_deprecated()) {
                    LogPrintf("%s: Updated zVDA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zvda seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvda wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVDAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVDACount(nCountLastUsed);
    else if (!walletdb.ReadZVDACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVDAWallet::Lock()
{
    seedMaster = 0;
}

void CzVDAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVDAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVDA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVDAWallet::LoadMintPoolFromDB()
{
...
void CzVDAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVDAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVDAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zvdaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVDAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVDA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zvdaTracker which also adds to database
    pwalletMain->zvdaTracker->Add(dMint, true);
...
void CzVDAWallet::SeedToZVDA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVDAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVDAWallet::GenerateDeterministicZVDA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVDAWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVDAWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzVDAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
