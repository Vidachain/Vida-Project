...
void CMasternodeMan::CountTiers(int protocolVersion, int& tier1, int& tier2)
{
    protocolVersion = protocolVersion == -1 ? masternodePayments.GetMinMasternodePaymentsProto() : protocolVersion;
...
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCount)
{
    ...
    std::vector<std::pair<int64_t, CTxIn>> vecMasternodeTiers[MasternodeTiers::TIER_NONE] = {};
    ...
    */
    ...
    for (CMasternode& mn : vMasternodes) {
        ...
        if (nBlockHeight < TIER_BLOCK_HEIGHT) {
            ...
            vecMasternodeTiers[mn.tier].push_back(std::make_pair(mn.SecondsSincePayment(), mn.vin));
        }
        nCount++;
    }
...
CMasternode* CMasternodeMan::GetWinningNode(std::vector<std::pair<int64_t, CTxIn>>& vecMasternodeLastPaid, uint256 blockHash)
{
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 20406) return;
        } else if (addr.GetPort() == 20406)
            return;
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 VIDA tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 VDA tx -> 1 confirmation
