...
public:
    ...
    bool WriteZVDASeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZVDASeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZVDASeed_deprecated(uint256& seed);
    bool EraseZVDASeed();
    bool EraseZVDASeed_deprecated();

    bool WriteZVDACount(const uint32_t& nCount);
    bool ReadZVDACount(uint32_t& nCount);
